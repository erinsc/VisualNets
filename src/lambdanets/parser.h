...
enum Type { Root, Alpha, Gamma, Delta, Epsilon, Lambda };

using term_map = std::map<std::string, std::string>;
using port_map = std::map<char, nodeid_t>;
using iss      = std::istringstream;
inline const char *reserved = "()\\@. \n\t\r\0";

constexpr Vector2 step_right = {NODE_RADIUS * 4, NODE_RADIUS * 4};
constexpr Vector2 step_down = {NODE_RADIUS * 4, 0};
